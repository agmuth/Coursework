...
train <- fread(file="Cleaned Data/train.csv")
train <- as.data.frame(train) #fread reads data in as a data.table must convert to data.frame for mlr

dist_mat1 <- fread(file="train_kmeans_dist.csv")
dist_mat1 <- as.data.frame(dist_mat1)

dist_mat2 <- fread(file="test_trees_kmeans_dist.csv")
dist_mat2 <- as.data.frame(dist_mat2)
dist_mat2 <- dist_mat2[,-2]

dist_mat <- rbind.data.frame(dist_mat1, dist_mat2)
dist_mat <- dist_mat[order(dist_mat$Id),]

rm(dist_mat1, dist_mat2)
...
for(i in 1:ncol(dist_mat)){
  ...
  saveRDS(tune_res, paste0("Final Model/tune_cluster", i, ".rds"))
  ...
  #save learner 
  saveRDS(learner_best, paste0("Final Model/xgboost_cluster", i, ".rds"))
  saveRDS(optCuts, paste0("Final Model/cuts_cluster", i, ".rds"))
