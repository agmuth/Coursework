...
class_wts <- get_weights(train$Response)
...
for(i in 9:ncol(dist_mat)){
  ...
  wts <- class_wts * wts #now weight by classes
  ...
  saveRDS(tune_res, paste0("Tunes/tune_all_wts_cluster", i, ".rds"))
  ...
  #save learner 
  saveRDS(learner_best, paste0("Models/xgboost_all_wts_cluster", i, ".rds"))
  t_stop <- proc.time()
