...
for(i in 1:8){
  ...
  train$Response <- factor(as.integer(train$Response == i))
  train  <- dplyr::select(train, -Id)
  
  #define task and learner
  task <- makeClassifTask(data=train, target="Response")
  ...
  SQWK_measure <- makeMeasure(id="SQWK_measure", minimize=FALSE, properties=c("classif", "classif.multi"), best=1, worst=0, 
                              fun=function(task, model, pred, feats, extra.args){return(SQWK_mlr_binary(pred))})
  ...
  saveRDS(tune_res, paste0("Tunes/tune_class", i, ".rds"))
  ...
  #save learner 
  saveRDS(learner_best, paste0("Models/xgboost_class", i, ".rds"))
}
